...
func (a *AuthHandler) Authenticate(c *gin.Context) {
	...
	if len(splits) == 2 {
		...
		c.AbortWithStatusJSON(403, ErrorResp{Error: "无效token"})
		return
	}
	uuid, isAdmin, err := a.TokenManager.ValidateToken(c, token)
	if err != nil {
		c.AbortWithStatusJSON(403, ErrorResp{Error: fmt.Sprintf("无法解析token，错误信息：%s", err.Error())})
		return
	}
...
func (a *AuthHandler) AuthorizeOwner(c *gin.Context) {
	if !c.GetBool("is_admin") {
		c.AbortWithStatusJSON(403, ErrorResp{Error: "未授权访问"})
	}
}

func (a *AuthHandler) BlockOwner(c *gin.Context) {
	if c.GetBool("is_admin") {
		c.AbortWithStatusJSON(403, ErrorResp{Error: "提问箱主人能问自己和其他提问箱主人问题嘛？答案是不能"})
		return
	}
}
